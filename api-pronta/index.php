...
/**
 * ⚠️ SEGURANÇA:
 * Você postou public/secret key no chat. TROQUE as chaves no painel e cole as novas aqui.
 * Eu deixei como PLACEHOLDER abaixo.
 */
...
$CONFIG = (object)[
    ...
    'CALLBACK_URL' => 'https://www.seusite.com/api-pronta/webhook.php',

    // Dados internos (não aparecem pro pagador)
    'CLIENT' => (object)[
        'name'     => 'NOME QUALQUER AQUI',
        'email'    => 'email@gmail.com',
        'phone'    => '00000000000',
        'document' => 'CPF QUALQUER AQUI',
    ],

    'PIX_HOLDER_FALLBACK' => 'NOME DA SUA EMPRESA INTERMEDIACOES LTDA',
    'CHECKOUT_MODEL'      => 'DESCRIÇÃO QUALQUER (sem preencher dados do pagador)',

    // PIXEL DO FACEBOOK DISPARA VARIOS EVENTOS IMPORTANTE PARA VENDER MAIS
    'FB_PIXEL_ID'         => 'DIGITE_AQUI_SEU_PIXEL_OU_QUALQUER_COISA',
...
/**
 * DEBUG:
 * - Para mostrar o retorno da API na tela, abra a página com ?debug=1
 *   Ex: /pix.php?debug=1
 */
...
if (isset($_GET['force_check']) && $_GET['force_check'] === '1') {
    ...
    if ($tid === '') {
        echo json_encode(['ok' => false, 'error' => 'TID inválido']);
        exit;
    }

    // Monta chamada para a API de transações da SigiloPay
    $url = rtrim($CONFIG->API_BASE, '/') . '/gateway/transactions';

    // Ajuste este payload conforme a documentação da /gateway/transactions
    $payload = [
    ...
    echo json_encode([
        ...
        'statusCode' => $httpCode,
        'raw'        => $resp,          // <-- AGORA MOSTRA RAW TAMBÉM
        'response'   => $json,
    ]);
    exit;
}
...
/* ===== DEBUG: CAPTURAR RESPOSTA DA API ===== */
$apiHttpCode  = null;
$apiRaw       = null;
$apiJson      = null;
$apiCurlErr   = null;
        ...
        if ($valor <= 0) {
            ...
            $payload = [
                ...
                'client'     => [
                    ...
                    'document' => $CONFIG->CLIENT->document,
                ],
            ];
            ...
            // ===== SALVA DEBUG SEMPRE (pra você ver com ?debug=1) =====
            $apiHttpCode = $httpCode;
            $apiRaw      = ($resp === false) ? null : (string)$resp;
            $apiCurlErr  = $curlErr ?: null;
            $apiJson     = ($resp !== false) ? json_decode((string)$resp, true) : null;
            ...
            if ($resp === false) {
                ...
                if (json_last_error() !== JSON_ERROR_NONE) {
                    ...
                    if (isset($json['transactionId']) && $json['transactionId'] !== '') {
                    ...
                    if (!$pixPayload) {
                        ...
                        if (!$qrImageSrc) {
                            ...
                            if ($qrBinary !== false) {
                                $qrImageSrc = 'data:image/png;base64,' . base64_encode($qrBinary);
                            } else {
                                $qrImageSrc = $qrUrl;
                            }
                        }
                        ...
                        if (!empty($info['holder'])) {
                            $pixHolderName = $info['holder'];
                        }
                        if (!empty($info['institution'])) {
                            $pixInstitutionName = $info['institution'];
                        }
...
// Pretty print seguro pro debug
$apiPretty = null;
if (is_array($apiJson)) {
    $apiPretty = json_encode($apiJson, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE);
}
?>
<!doctype html>
<html lang="pt-BR">
<head>
  ...
  <style>
    ...
    *{box-sizing:border-box;}
    ...
    body[data-theme="light"] .debug pre{
    ...
    /* Botão "Já paguei" começa escondido */
    #btnJaPaguei{
      display:none;
      margin-top:8px;
    }
...
<body data-theme="dark">
    ...
    <div class="card">
      <?php if ($hasPix): ?>
        <div id="pixStep">
          ...
          <div class="pix-box">
            ...
            <!-- Botão "Já paguei" (aparece 20s após gerar o PIX) -->
            <button type="button" id="btnJaPaguei" class="secondary">
              Já paguei, verificar pagamento
            </button>

            <button type="button" class="secondary" onclick="window.location.href=window.location.pathname<?= $DEBUG ? "+'?debug=1'" : "" ?>;">
              Gerar nova cobrança
            </button>
          </div>
        ...
        <div id="paidStep">
          ...
          <div class="paid-sub">
            PIX de R$ <?= $amountDisplay ? e($amountDisplay) : '—' ?> confirmado.
          </div>
          ...
          <div class="small">
            Liberado o acesso/serviço ao cliente.
          </div>
      ...
      <?php else: ?>
        ...
        <div class="small">
          ...
          Checkout rápido sem preencher dados do pagador.
        </div>
  ...
  <script>
    ...
    (function setupJaPaguei(){
      ...
      const INITIAL_DELAY_MS  = 20000; // 20 segundos para aparecer
      const COOLDOWN_DELAY_MS = 20000; // 20 segundos desativado após cada clique
      ...
      btn.addEventListener('click', async () => {
        ...
        try {
          const url = window.location.pathname + '?force_check=1&tid=' + encodeURIComponent(TID);
    ...
    (function startPaymentWatcher(){
      ...
      let timer = setInterval(async () => {
        try {
          const url = window.location.pathname + '?check=1&tid=' + encodeURIComponent(TID);
