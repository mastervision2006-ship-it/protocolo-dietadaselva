...
// Agora:
//  - NÃO bloqueia por token (só loga)
...
// Garante que a pasta /pagamentos existe
if (!is_dir($DIR)) {
    mkdir($DIR, 0775, true);
}
...
/* ===========================
   SÓ ACEITA POST COM BODY
   =========================== */
if (($_SERVER['REQUEST_METHOD'] ?? '') !== 'POST') {
    wlog('MÉTODO INVÁLIDO: ' . ($_SERVER['REQUEST_METHOD'] ?? ''));
...
/*
    ...
    - TRANSACTION_CREATED (PENDING) → NÃO salva arquivo, só loga.
    - TRANSACTION_PAID + status COMPLETED → AÍ sim salva o JSON com o transaction.id.

    Assim, a index.php só vai aprovar quando de fato estiver pago.
*/
if (
    !in_array($event, ['TRANSACTION_PAID', 'TRANSACTION_COMPLETED'], true) &&
    $status !== 'COMPLETED'
) {
    wlog('IGNORADO (NÃO PAGO): event=' . json_encode($event) . ' status=' . json_encode($status));
    http_response_code(200);
    header('Content-Type: application/json; charset=utf-8');
...
if ($tidClean === '') {
    wlog('transaction.id INVÁLIDO: ' . json_encode($tid));
    http_response_code(200);
    header('Content-Type: application/json; charset=utf-8');
...
/* ===========================
   SALVAR ARQUIVO DA TRANSAÇÃO PAGA
   =========================== */
